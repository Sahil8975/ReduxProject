...
function CustomerNameDialog({
  ...
  setCustomerTimeBasedOriginalCollector,
  payloadData,
  setPayloadData,
  collectorsList,
  isEdit
  ...
  const myCloseModal = (event, reason) => {
    ...
    setCustomerTimeBasedOriginalCollector(copyArray);
    setNonEditable(false);
    setIsRecordEdit(false);
    handleClose();
  };

  const arrayLength = customerTimeBasedOriginalCollector.length;

  const getToDate = (fromDate) => {
    const futureDate = moment(fromDate).add(50, 'years');
    const indOfCurrentObj = customerTimeBasedOriginalCollector.findIndex(
    ...
    if (indOfCurrentObj !== -1 && arrayLength > indOfCurrentObj + 1) {
      const dateObj = customerTimeBasedOriginalCollector[indOfCurrentObj + 1].validFrom;
      return moment(dateObj).subtract(1, 'days').format('YYYY-MM-DDTHH:mm:ss');
    }
    return futureDate.format('YYYY-MM-DDTHH:mm:ss');
  };

  const selectedObj = customerTimeBasedOriginalCollector.find((itm) => itm.isSelected);

  const handleOkClick = () => {
    const { validFrom, fieldValue } = payload;
    const selectedIndex = customerTimeBasedOriginalCollector.findIndex((itm) => itm.isSelected === true);
    const isObjectFound = customerTimeBasedOriginalCollector.some(
      (item) =>
        item.fieldValue === fieldValue.toString() &&
        moment(item.validFrom).format('YYYY-MM-DD') === moment(validFrom).format('YYYY-MM-DD')
    );
    const unsavedObj = customerTimeBasedOriginalCollector.some((item) => item.id === 0 && item.isSelected === true);
    ...
    if (!isObjectFound) {
      const modifiedData = [...customerTimeBasedOriginalCollector];
      if (isRecordEdit) {
        ...
        if (moment(validFrom).format('YYYY-MM-DD') === moment(selectedObj.validFrom).format('YYYY-MM-DD')) {
          ...
          const index = customerTimeBasedOriginalCollector.findIndex(
        ...
        setCustomerTimeBasedOriginalCollector(modifiedData);
        const currentObj = modifiedData.find((itm) => itm.isSelected);
        const { fieldValue: currFieldValue } = currentObj;
        setPayloadData({ ...payloadData, collectorId: currFieldValue * 1 });
      }
    }
    handleProceed();
    setNonEditable(false);
  ...
  useEffect(() => {
    ...
    const futureRecords = hasFutureRecords(customerTimeBasedOriginalCollector);
    const previousRecords = hasPreviousRecords(customerTimeBasedOriginalCollector);
  ...
  const handleOkEditedRecord = () => {
    ...
    if (index !== -1) {
      ...
      if (index === 0) {
        setPayloadData({ ...payloadData, collectorId: payloadfieldValue * 1 });
      }
    } else {
      updatePayload({
        fieldValue: selectedObj.fieldValue,
  ...
  // console.log('customerTimeBasedOriginalCollector', customerTimeBasedOriginalCollector);

  useEffect(() => {
    if (isRecordEdit) {
      const { validFrom } = payload;
      const newInd = customerTimeBasedOriginalCollector.findIndex(
  ...
  const getMaxFromDate = () => {
    ...
    const selectedIndex = customerTimeBasedOriginalCollector.findIndex((obj) => obj.isSelected);
    ...
    if (arrayLength === 1) {
      ...
      const newDate = new Date(customerTimeBasedOriginalCollector[selectedIndex + 1]?.validFrom);
  ...
  const getMinFromDate = () => {
    ...
    if (selectedObj.id === 0 && arrayLength === 1) {
      ...
      const newDate = new Date(customerTimeBasedOriginalCollector[selectedIndex]?.validFrom);
      newDate.setDate(newDate.getDate() + 1);
      minDate = newDate;
    } else if (isRecordEdit && isFutureData) {
      if (isPastData) {
        const prevDate = new Date(customerTimeBasedOriginalCollector[selectedIndex - 1]?.validFrom);
        prevDate.setDate(prevDate.getDate() + 1);
        minDate = prevDate;
      } else if (isFutureData && arrayLength === 2) {
        const prevDate = new Date(customerTimeBasedOriginalCollector[selectedIndex]?.validFrom);
        prevDate.setDate(prevDate.getDate() + 1);
        minDate = prevDate;
      } else {
        minDate = undefined;
  ...
  useEffect(() => {
    if (isEdit && isArray(collectorsList)) {
      const tableData = [];
      customerTimeBasedOriginalCollector.map((itm) => {
        const { validFrom, validTo, fieldValue } = itm;
        if (fieldValue) {
          const collector = collectorsList.find((s) => s.id === fieldValue * 1);
          if (collector) {
  ...
  }, [customerTimeBasedOriginalCollector, collectorsList]);

  useEffect(() => {
    const { collectorId } = payloadData;
    const { validFrom } = selectedObj;
    updatePayload({ fieldValue: collectorId * 1, validFrom });
    setCopyPayload({ ...copyPayload, collectorId });
    setCopyArray(customerTimeBasedOriginalCollector);
  }, [payloadData]);
  ...
  return (
    <Dialog open={open} onClose={myCloseModal} fullWidth maxWidth="md">
      ...
      <DialogTitle>{isEdit ? 'Original Collector Name' : 'Add Original Collector Name'}</DialogTitle>
      <DialogContent>
...
CustomerNameDialog.propTypes = {
  ...
  collectorsList: PropTypes.array,
  customerTimeBasedOriginalCollector: PropTypes.array,
  setCustomerTimeBasedOriginalCollector: PropTypes.func,
