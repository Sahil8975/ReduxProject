...
function CustomerArabicNameDialog({
  open,
  handleClose,
  handleProceed,
  customerTimeBasedArabicName,
  setCustomerTimeBasedArabicName,
  payloadData,
  setPayloadData,
  isEdit
}) {
  ...
  const myCloseModal = (event, reason) => {
    ...
    setCustomerTimeBasedArabicName(copyArray);
    setNonEditable(false);
    setIsRecordEdit(false);
    handleClose();
  };

  const selectedObj = customerTimeBasedArabicName.find((itm) => itm.isSelected);

  const arrayLength = customerTimeBasedArabicName.length;

  const getToDate = (fromDate) => {
    const futureDate = moment(fromDate).add(50, 'years');
    const indOfCurrentObj = customerTimeBasedArabicName.findIndex(
    ...
    if (indOfCurrentObj !== -1 && arrayLength > indOfCurrentObj + 1) {
      const dateObj = customerTimeBasedArabicName[indOfCurrentObj + 1].validFrom;
  ...
  const handleOkClick = () => {
    ...
    const selectedIndex = customerTimeBasedArabicName.findIndex((itm) => itm.isSelected === true);
    const isObjectFound = customerTimeBasedArabicName.some(
      (item) =>
        item.fieldValue === fieldValue &&
        moment(item.validFrom).format('YYYY-MM-DD') === moment(validFrom).format('YYYY-MM-DD')
    );
    const unsavedObj = customerTimeBasedArabicName.some((item) => item.id === 0 && item.isSelected === true);
    ...
    if (!isObjectFound) {
      const modifiedData = [...customerTimeBasedArabicName];
      if (isRecordEdit) {
        ...
        if (moment(validFrom).format('YYYY-MM-DD') === moment(selectedObj.validFrom).format('YYYY-MM-DD')) {
          ...
          const index = customerTimeBasedArabicName.findIndex((item) => item.id === 0 && item.isSelected === true);
          if (index > 0) {
        ...
        setCustomerTimeBasedArabicName(modifiedData);
        const currentObj = modifiedData.find((itm) => itm.isSelected);
        const { fieldValue: currFieldValue } = currentObj;
        setPayloadData({ ...payloadData, customerArabicName: currFieldValue });
      }
    }
    handleProceed();
    setNonEditable(false);
  ...
  useEffect(() => {
    ...
    const futureRecords = hasFutureRecords(customerTimeBasedArabicName);
    const previousRecords = hasPreviousRecords(customerTimeBasedArabicName);
  ...
  const handleOkEditedRecord = () => {
    ...
    const index = customerTimeBasedArabicName.findIndex(
      (item) =>
        item.fieldValue === fieldValue &&
    ...
    if (index !== -1) {
      ...
      if (index === 0) {
        setPayloadData({ ...payloadData, customerArabicName: payloadfieldValue });
      }
    } else {
      updatePayload({
        fieldValue: selectedObj.fieldValue,
  ...
  // console.log('customerTimeBasedArabicName', customerTimeBasedArabicName);

  useEffect(() => {
    if (isRecordEdit) {
      const { validFrom } = payload;
      const newInd = customerTimeBasedArabicName.findIndex(
  ...
  const getMaxFromDate = () => {
    ...
    const selectedIndex = customerTimeBasedArabicName.findIndex((obj) => obj.isSelected);
    ...
    if (arrayLength === 1) {
      ...
      const newDate = new Date(customerTimeBasedArabicName[selectedIndex + 1]?.validFrom);
  ...
  const getMinFromDate = () => {
    ...
    if (selectedObj.id === 0 && arrayLength === 1) {
      ...
      const newDate = new Date(customerTimeBasedArabicName[selectedIndex]?.validFrom);
      newDate.setDate(newDate.getDate() + 1);
      minDate = newDate;
    } else if (isRecordEdit && isFutureData) {
      if (isPastData) {
        const prevDate = new Date(customerTimeBasedArabicName[selectedIndex - 1]?.validFrom);
        prevDate.setDate(prevDate.getDate() + 1);
        minDate = prevDate;
      } else if (isFutureData && arrayLength === 2) {
        const prevDate = new Date(customerTimeBasedArabicName[selectedIndex]?.validFrom);
        prevDate.setDate(prevDate.getDate() + 1);
        minDate = prevDate;
      } else {
        minDate = undefined;
  ...
  const components = [
    {
      ...
      columnWidth: 6,
      isRequired: false,
      isError: (error && payload.fieldValue.length > 130) || nonEditable,
      helperText:
        (error && 'Customer arabic name must be 130 characters long.') ||
        (nonEditable && 'This record cannot be edited'),
      groupStyle: { fontSize: '1.2rem' }
      // isDisabled:
    },
    {
      control: DATEPICKER,
  ...
  useEffect(() => {
    if (isEdit) {
      const tableData = [];
      customerTimeBasedArabicName.map((itm) => {
  ...
  useEffect(() => {
    const { customerArabicName } = payloadData;
    const { validFrom } = selectedObj;
    updatePayload({ fieldValue: customerArabicName, validFrom });
    setCopyPayload({ ...copyPayload, customerArabicName });
    setCopyArray(customerTimeBasedArabicName);
  }, [payloadData]);
  ...
  return (
    <Dialog open={open} onClose={myCloseModal} fullWidth maxWidth="md">
      ...
      <DialogTitle>{isEdit ? 'Customer Name (Arabic)' : 'Add Customer Name (Arabic)'}</DialogTitle>
      <DialogContent>
          ...
          <Grid item xs={12}>
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 650 }}>
                <TableHead>
                  <TableRow>
                    <TableCell>Customer Name (Arabic)</TableCell>
...
CustomerArabicNameDialog.propTypes = {
  open: PropTypes.bool,
  handleClose: PropTypes.func,
  handleProceed: PropTypes.func,
  payloadData: PropTypes.object,
  setPayloadData: PropTypes.func,
  customerTimeBasedArabicName: PropTypes.array,
  setCustomerTimeBasedArabicName: PropTypes.func,
  isEdit: PropTypes.bool
};

export default CustomerArabicNameDialog;
