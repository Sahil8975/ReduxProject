...
function CustomerNameDialog({
  ...
  const myCloseModal = (event, reason) => {
    ...
    setCustomerTimeBasedName(copyArray);
    setNonEditable(false);
    setIsRecordEdit(false);
    handleClose();
  };

  const selectedObj = customerTimeBasedName.find((itm) => itm.isSelected);

  const arrayLength = customerTimeBasedName.length;

  const getToDate = (fromDate) => {
    const futureDate = moment(fromDate).add(50, 'years');
    const indOfCurrentObj = customerTimeBasedName.findIndex(
    ...
    if (indOfCurrentObj !== -1 && arrayLength > indOfCurrentObj + 1) {
      const dateObj = customerTimeBasedName[indOfCurrentObj + 1].validFrom;
  ...
  const handleOkClick = () => {
    ...
    const selectedIndex = customerTimeBasedName.findIndex((itm) => itm.isSelected === true);
    const isObjectFound = customerTimeBasedName.some(
      (item) =>
        item.fieldValue === fieldValue &&
        moment(item.validFrom).format('YYYY-MM-DD') === moment(validFrom).format('YYYY-MM-DD')
    );
    const unsavedObj = customerTimeBasedName.some((item) => item.id === 0 && item.isSelected === true);
    ...
    if (!isObjectFound) {
      const modifiedData = [...customerTimeBasedName];
      if (isRecordEdit) {
        ...
        if (moment(validFrom).format('YYYY-MM-DD') === moment(selectedObj.validFrom).format('YYYY-MM-DD')) {
          ...
          const index = customerTimeBasedName.findIndex((item) => item.id === 0 && item.isSelected === true);
          if (index > 0) {
  ...
  // console.log('customerTimeBasedName', customerTimeBasedName);
  ...
  useEffect(() => {
    ...
    const futureRecords = hasFutureRecords(customerTimeBasedName);
    const previousRecords = hasPreviousRecords(customerTimeBasedName);
  ...
  const handleOkEditedRecord = () => {
    ...
    const index = customerTimeBasedName.findIndex(
      (item) =>
        item.fieldValue === fieldValue &&
    ...
    if (index !== -1) {
      ...
      if (index === 0) {
        setPayloadData({ ...payloadData, customerName: payloadfieldValue.toString() });
      }
    } else {
      updatePayload({
        fieldValue: selectedObj.fieldValue,
  ...
  useEffect(() => {
    if (isRecordEdit) {
      ...
      const newInd = customerTimeBasedName.findIndex(
  ...
  const getMaxFromDate = () => {
    ...
    const selectedIndex = customerTimeBasedName.findIndex((obj) => obj.isSelected);
    ...
    if (arrayLength === 1) {
      ...
      const newDate = new Date(customerTimeBasedName[selectedIndex + 1]?.validFrom);
  ...
  const getMinFromDate = () => {
    ...
    if (selectedObj.id === 0 && arrayLength === 1) {
      ...
      const newDate = new Date(customerTimeBasedName[selectedIndex]?.validFrom);
      newDate.setDate(newDate.getDate() + 1);
      minDate = newDate;
    } else if (isRecordEdit && isFutureData) {
      if (isPastData) {
        const prevDate = new Date(customerTimeBasedName[selectedIndex - 1]?.validFrom);
        prevDate.setDate(prevDate.getDate() + 1);
        minDate = prevDate;
      } else if (isFutureData && arrayLength === 2) {
        const prevDate = new Date(customerTimeBasedName[selectedIndex]?.validFrom);
        prevDate.setDate(prevDate.getDate() + 1);
        minDate = prevDate;
      } else {
        minDate = undefined;
  ...
  const components = [
    {
      ...
      label: 'Customer Name',
      columnWidth: 6,
      isError: (error && payload?.fieldValue.length > 100) || nonEditable,
      helperText:
        (error && 'Customer Name must be 100 characters long') || (nonEditable && 'This record cannot be edited'),
      isRequired: true
    },
    {
      control: DATEPICKER,
  ...
  useEffect(() => {
    const { customerName } = payloadData;
    const { validFrom } = selectedObj;
    updatePayload({ fieldValue: customerName, validFrom });
    setCopyPayload({ ...copyPayload, customerName });
    setCopyArray(customerTimeBasedName);
  }, [payloadData]);
  ...
  return (
    <Dialog open={open} onClose={myCloseModal} fullWidth maxWidth="md">
      ...
      <DialogTitle>{isEdit ? 'Customer Name' : 'Add Customer Name'}</DialogTitle>
      <DialogContent>
...
CustomerNameDialog.propTypes = {
  ...
  customerTimeBasedName: PropTypes.array,
  setCustomerTimeBasedName: PropTypes.func,
